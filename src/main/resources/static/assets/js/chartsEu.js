...
$.ajax({
    ...
    success:function (data) {
        ...
        console.log('tmpEu: ' + (new Date(tmpEu[0][0])).getDay());
    }
});
...
function generateDatas2Eu(input1, input2) {
    var data = [];
    for (var j = 0; j < tmpEu.length; ++j) {
        if (input1 === 'all' && input2 === 'all') {
            data.push(generateData3Eu(j));
        } else if (input1 === 'all' && input2 !== 'all') {
            if ((new Date(tmpEu[j][0])).getDay() === input2) {
                data.push(generateData3Eu(j));
            }
        }
    }
    for (var i = 0; i < tmp3Eu.length; ++i) {
        if (input1 !== 'all' && input2 === 'all') {
            if (tmp3Eu[i][1] === input1){
                data.push(generateData2Eu(i));
            }
        }
        else if (input1 !== 'all' && input2 !== 'all'){
            if (tmp3Eu[i][1] === input1 && (new Date(tmp3Eu[i][0])).getDay() === input2){
                data.push(generateData2Eu(i));
            }
        }
    }
...
function draw_chart_by_NameEu(){
    ...
    var data2 = generateDatas2Eu(inputData,inputData2);
...
var data2Eu = generateDatas2Eu('all','all');
series.data.setAll(data2Eu);
